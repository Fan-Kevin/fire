...
int coco_detect_run(rknn_context ctx, cv::Mat input_image, coco_detect_result_group_t *detect_result_group)
{
	...
	// sprintf(detect_result_group->results[0].name, "%s", "car");
	// detect_result_group->results[0].class_index = 1;
	// detect_result_group->results[0].prop = 0.9;
	// detect_result_group->results[0].box.left = 1190;
	// detect_result_group->results[0].box.top = 174;
	// detect_result_group->results[0].box.right = 1607;
	// detect_result_group->results[0].box.bottom = 538;
	// return 1;;
	...
	*/
	...
	std::cout << "buf type " << typeid(letter_image.data[0]).name() << endl;
	// printf(" letter_image.data   dtype %s,\n",typeid(letter_image.data).name  );
